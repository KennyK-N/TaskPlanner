...
def task_finalize():
    ...
    if request.method == "POST":
        task_list = (
            request.values.get("itemsInput")
            if request.values.get("itemsInput") != None
            else None
        )

        clean = sanitize_utils.clean_value(task_list)

        task_list = [task.strip() for task in clean.split(",") if task.strip() != ""]
        ...
        if (task_list_sz > MAX_TASK_LIST_SIZE or task_list_sz == 0) or (
            not task_list or not date or not schedule_name
        ):
        ...
        return render_template(
            "task_finalize.html",
            Generated_Tasks=generated_response,
            schedule_for=date,
...
def create_event():
    ...
    if request.method == "POST":
        ...
        if (
            ...
            or len(time_begins) != len(time_end)
            or not task_names
        ):
            redirect_message = {"success": False, "message": "Invalid Input"}
            flash(redirect_message)
        ...
        isViolatedTime = task_util.task_violate_time_check(
            time_begins=time_begins, time_end=time_end
        )
        ...
        status = google_calendar_.create_calendar(task_list=task_list, date=date)
...
def create_schedule():
    ...
    if status is general_utils.CalendarStatus.EMPTY:
        return jsonify(
            {"success": False, "error": "Failed to create due to empty calendar"}
        )

    return jsonify(
        {"success": True, "message": "Successfully created a calendar for current task"}
    )
