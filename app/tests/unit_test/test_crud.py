...
def test_retrieve_single_item_success(
    mocker, mock_db_session, mock_db_profile_id, mock_db_redis_set, mock_db_redis_get
):
    """Fetches a task from the DB by id and returns it as a parsed dict."""
    mock_obj = SimpleNamespace(id=1, name="Test", data=json.dumps({"test": "works"}))
...
def test_retrieve_single_item_redis_success(
    mocker, mock_db_session, mock_db_profile_id, mock_db_redis_set, mock_db_redis_get
):
    """Returns a cached task from Redis without hitting the database."""
    mock_obj = {"id": 1, "name": "Test", "data": {"test": "works"}}
...
def test_insert_success(
    mocker, mock_task_object, mock_db_session, mock_db_profile_id, mock_db_redis_set
):
...
def test_retrieve_tasks_success(
    mocker, mock_db_profile_id, mock_task_object, mock_db_redis_set, mock_db_redis_get
):
...
def test_retrieve_tasks_redis_success(
    mocker, mock_db_profile_id, mock_task_object, mock_db_redis_set, mock_db_redis_get
):
...
def test_delete_task_success(
    mocker, mock_task_object, mock_db_profile_id, mock_db_session
):
...
def test_insert_fail(mocker, mock_task_object, mock_db_session, mock_db_profile_id):
