...
def test_get_addr_success_redis(
    mocker, mock_geocoding_profile_id, mock_geocoding_redis_get
):
...
def test_gemini_prompt_success(mocker, flask_app_mock):
    ...
    flask_app_mock.extensions[
        "gemini_CLIENT"
    ].models.generate_content.return_value.text = '{"1": 1}'
...
def test_gemini_prompt_fail(mocker, flask_app_mock):
    ...
    flask_app_mock.extensions["gemini_CLIENT"].models.generate_content.side_effect = (
...
def test_calendar_success(mocker):
    ...
    oauth_mock = mocker.patch(
        "app.api.Google_Calendar.google_calendar_.google.oauth2.credentials.Credentials"
    )
    mocker.patch(
        "app.api.Google_Calendar.google_calendar_.session", {"credentials": MagicMock()}
    )
    ...
    service_mock = mocker.patch(
        "app.api.Google_Calendar.google_calendar_.googleapiclient.discovery",
        return_value=MagicMock(),
    )
    service_mock.build.return_value.events.return_value.insert.return_value.execute.return_value = (
        MagicMock()
    )
...
def test_calendar_fail_invalid_input(mocker):
    ...
    service_mock.build.return_value.return_value.insert.return_value.execute.return_value = (
        MagicMock()
    )
    service_mock.events.insert.execute.get.return_value = "Testing Works"
...
def test_calendar_fail_service_fail(mocker):
    ...
    service_mock.build.return_value.events.return_value.insert.return_value.execute.return_value.get.side_effect = Exception(
        "Failed"
    )
