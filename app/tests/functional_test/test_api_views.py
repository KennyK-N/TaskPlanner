...
def test_prompt_success(mocker, client):
    ...
    mocker.patch(
        "app.api.views.permission_utils.check_calendar_access", return_value=True
    )
...
def test_fail_prompt_HasCalendarAccess_fail(mocker, client):
    ...
    mocker.patch("app.api.views.permission_utils.check_view_access", return_value=True)
    mocker.patch(
        "app.api.views.permission_utils.check_calendar_access", return_value=False
    )
    response = client.get("/prompt")
...
def test_task_finalize_invalid_input_empty_task_list(mocker, client):
    ...
    response = client.post(
        "/task_finalize",
        data={"itemsInput": "", "date": "2025-01-01", "schedule_name": "Test"},
    )
...
def test_task_finalize_invalid_input_too_many_tasks(mocker, client):
    ...
    mocker.patch(
        "app.api.views.sanitize_utils.clean_value", return_value="a, b, c, d, e, f"
    )
...
def test_task_finalize_gemini_returns_none(mocker, client):
    ...
    mocker.patch(
        "app.api.views.sanitize_utils.clean_value", return_value="task1, task2"
    )
...
def test_task_finalize_success(mocker, client):
    ...
    mocker.patch(
        "app.api.views.gemini_.generate_tasks", return_value=[{"task_name": "task1"}]
    )
...
def test_create_event_invalid_input_missing_date(mocker, client):
    ...
    mocker.patch(
        "app.api.views.sanitize_utils.clean_value",
        return_value=[
            "t",
        ],
    )
    mocker.patch(
        "app.api.views.sanitize_utils.clean_list",
        return_value=[
            "task1",
        ],
    )
...
def test_create_event_time_violation(mocker, client):
    ...
    mocker.patch(
        "app.api.views.sanitize_utils.clean_value", side_effect=["2025-01-01", "Test"]
    )
    mocker.patch(
        "app.api.views.sanitize_utils.clean_list",
        side_effect=[["task1"], ["desc"], ["10:00"], ["09:00"], ["None"]],
    )
...
def test_create_event_calendar_revoke(mocker, client):
    ...
    mocker.patch(
        "app.api.views.sanitize_utils.clean_list",
        side_effect=[["task1"], ["desc"], ["09:00"], ["10:00"], ["None"]],
    )
    ...
    mocker.patch(
        "app.api.views.google_calendar_.create_calendar",
        return_value=CalendarStatus.REVOKE,
    )
...
def test_create_event_calendar_empty(mocker, client):
    ...
    mocker.patch(
        "app.api.views.google_calendar_.create_calendar",
        return_value=CalendarStatus.EMPTY,
    )
...
def test_create_event_success(mocker, client):
    ...
    mocker.patch(
        "app.api.views.google_calendar_.create_calendar",
        return_value=CalendarStatus.SUCCESS,
    )
...
def test_create_schedule_calendar_revoke(mocker, client):
    ...
    mocker.patch(
        "app.api.views.db_services.retrieve_single_item",
        return_value={"data": [], "name": "Test"},
    )
    mocker.patch(
        "app.api.views.google_calendar_.create_calendar",
        return_value=CalendarStatus.REVOKE,
